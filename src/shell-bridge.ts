...
function renderFishBridge(eol: string): string {
  ...
  const lines = [
    ...
    "  set first $argv[1]",
    "  switch $first",
    `    case '/supercodex:*' 'supercodex:*' '/sc:*' 'sc:*' ${commandCases}`,
    "      supercodex $argv",
    "    case '*'",
    "      set -e argv[1]",
    "      supercodex \"supercodex:$first\" $argv",
    "  end",
    "end",
    END_MARKER
  ];
...
function renderPowerShellBridge(eol: string): string {
  ...
  const lines = [
    ...
    "  }",
    "",
    "  supercodex (\"supercodex:$first\") @rest",
    "}",
    END_MARKER
  ];
