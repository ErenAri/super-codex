...
npm dist-tag add supercodex@2.1.0-beta.1 next
```
...
- Manual dispatch supports two lanes:
  - `canary` (requires prerelease version such as `2.1.0-beta.1`)
...
```bash
npm run release:publish:verify -- --version 2.0.0 --dist-tag latest
```
